...
const fallbackDb = {
  collection(name) {
    ...
    return {
      find(filter = {}) {
        const createCursor = (options = {}, limitVal = null) => ({
          sort(sortValue = {}) {
            return createCursor({ ...options, sort: sortValue }, limitVal);
          },
          limit(limitValue) {
            return createCursor(options, limitValue);
          },
          async toArray() {
            let res = toArrayFromCollection(name, filter, options);
