...
export async function postToThreads({ threadsUserId, accessToken, text, mediaUrl, isVideo }) {
  ...
  if (!createRes.ok) {
    if (createData?.error?.code === 190) {
      throw new Error("Threads token invalid or expired. Please reconnect your Threads profile on the Social Channels (/accounts) page.");
    }
