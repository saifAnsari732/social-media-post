...
export async function postToInstagram({ igUserId, accessToken, mediaUrl, caption, isVideo, disableComments }) {
  ...
  if (disableComments && publishData.id) {
    try {
      await fetch(`https://graph.facebook.com/v20.0/${publishData.id}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      console.log(`[Instagram] Comments disabled for media ${publishData.id}`);
    } catch (commentErr) {
