...
export async function scanCopyrightAI({
  ...
  // Combined: file/URL & title are primary signals
  const isAIContent = isAIFromFile || isAIFromTitle || isAIFromCaption;
  ...
  let audioRisk = isVideo ? 10 + ((hashNum + 2) % 12) : 4;
  const issues = [];
...
4. Audio signals (if video):
   - original vs third-party audio
   - music presence (commercial / background)
   - voice type (human vs synthetic TTS)
   - audio_risk: 0–100 (0 = fully original, 100 = high commercial music risk)
   - audio_type: "original_human" | "synthetic_tts" | "commercial_music" | "royalty_free" | "unknown"
...
- Creator/Brand: "${ownerBusiness}"`;
      ...
      // Build parts for Gemini (text + media URL inline)
      ...
      if (mediaUrl) {
        ...
        parts.push({ fileData: { mimeType, fileUri: mediaUrl } });
      }
...
Return ONLY JSON:
{
  ...
  if (geminiEvidence) {
    ...
    const rawAudioRisk = typeof au.audio_risk === "number" ? au.audio_risk : 0;
    const syntheticTTSPenalty = au.audio_type === "synthetic_tts" ? 15 : 0;
    const commercialMusicPenalty = au.audio_type === "commercial_music" ? 40 : 0;
    const adjustedAudioRisk = Math.min(100, rawAudioRisk + syntheticTTSPenalty + commercialMusicPenalty);
    ...
    if (isVideo) {
      audioRisk = Math.min(100, Math.max(audioRisk * 0.3, adjustedAudioRisk));
    }
    ...
    if (!aiAudioFindings) aiAudioFindings = evidenceByCategory("audio") || (au.audio_type ? `Audio type: ${au.audio_type}` : null);
  ...
  const result = {
    ...
    detectedMediaOrigin: (() => {
      if (geminiEvidence) {
        ...
        if (aiLikelihood > 60 || deepfakeLikelihood > 50) return "ai";
        if (aiLikelihood > 30) return "ai_assisted";
        return "real";
      }
      return isAIContent ? "ai" : "real";
    })(),
    mediaOriginConfidence: (() => {
      if (geminiEvidence) {
        const aiLikelihood = geminiEvidence.video_analysis?.ai_generated_likelihood ?? 0;
        const deepfakeLikelihood = geminiEvidence.video_analysis?.deepfake_likelihood ?? 0;
        // High confidence when Gemini clearly identifies one way or another
        const maxSignal = Math.max(aiLikelihood, deepfakeLikelihood);
        if (maxSignal > 70) return Math.round(85 + maxSignal * 0.15);
        if (maxSignal > 40) return Math.round(65 + maxSignal * 0.3);
        return Math.round(90 - maxSignal * 0.4); // High confidence it's real
      }
      return isAIFromFile ? 92 : (isAIFromCaption ? 78 : 97);
    })(),
    contentAnalyzer: {
      text: {
        riskScore: textRisk,
