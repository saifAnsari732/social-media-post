...
Return ONLY JSON:
{
  ...
  if (geminiEvidence) {
    ...
    const isAIDetected = aiGenLikelihood >= 20 || deepfakeLikelihood >= 20 || isAIContent || isAIFromVisualSignal;
    if (isAIDetected) {
      if (!hasAIDisclosure) {
        videoRisk = Math.max(videoRisk, 52);
        imageRisk = Math.max(imageRisk, 45);
        ...
        if (!alreadyHasFlag) {
          issues.push({
            ...
            severity: "medium",
            title: "Missing Platform AI Transparency Label",
            desc: `AI-generated / synthetic media detected (${Math.max(aiGenLikelihood, deepfakeLikelihood, 80)}% confidence). Meta AI Info & YouTube Altered Content policies require transparent disclosure.`,
            fixType: "ai_disclosure"
          });
        }
      } else {
