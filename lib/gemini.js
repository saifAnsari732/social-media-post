...
export async function scanCopyrightAI({
  ...
  const isAIMarkerPresent = aiMarkers.some(m => corpus.includes(m));
  const hasAIDisclosure = corpus.includes("ai & synthetic media disclosure") || corpus.includes("aigenerated") || corpus.includes("syntheticmedia");
  ...
  if (isAIMarkerPresent && !hasAIDisclosure) {
    imageRisk += 30;
    videoRisk += 20;
    issues.push({
...
Return ONLY JSON:
{
  ...
  const result = {
    ...
    detectedMediaOrigin: isAIMarkerPresent ? "ai" : "real",
    mediaOriginConfidence: isAIMarkerPresent ? 96 : 99,
    contentAnalyzer: {
      text: {
        riskScore: textRisk,
