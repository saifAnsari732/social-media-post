...
export async function updateUserPlan(userId, planName, transactionDetails = {}) {
  ...
  const user = await db.collection("users").findOne({ 
    $or: [{ userId: cleanId }, { email: cleanId.toLowerCase() }] 
  });

  const isPro = planName.toLowerCase().includes("pro") || planName.toLowerCase().includes("unlimited");
  const isGrowth = planName.toLowerCase().includes("growth");
  const baseMonthlyPrice = isPro ? 7999 : isGrowth ? 4999 : 2999;

  const cycle = transactionDetails.billingCycle || "monthly";
  const defaultOriginal = cycle === "yearly" ? baseMonthlyPrice * 12 : baseMonthlyPrice;

  const originalAmount = transactionDetails.originalAmount !== undefined && transactionDetails.originalAmount > 0 
    ? transactionDetails.originalAmount 
    : defaultOriginal;

  const amountPaid = transactionDetails.amountPaid !== undefined 
    ? transactionDetails.amountPaid 
    : originalAmount;

  const discountAmount = transactionDetails.discountAmount !== undefined 
    ? transactionDetails.discountAmount 
    : Math.max(0, originalAmount - amountPaid);

  const durationDays = cycle === "yearly" ? 365 : 30;

  const userEmail = (user?.email || cleanId).toLowerCase();

  // Update ALL user documents matching userId OR email
  await db.collection("users").updateMany(
    ...
    { 
      $set: { 
        plan: planName,
        isPaid: true,
        trialStartDate: null,      // Remove trial status completely!
  ...
  // Record transaction in billing_history collection with full invoice details for Admin & User
  if (transactionDetails.orderId || transactionDetails.paymentId) {
    ...
    await db.collection("billing_history").insertOne({
      invoiceId,
      userId: cleanId,
      userName: user?.name || cleanId.split("@")[0] || "Subscriber",
      userEmail: userEmail,
      planName,
      billingCycle: cycle,
      originalAmount,
  ...
  return db.collection("users").findOne({ 
    $or: [{ userId: cleanId }, { email: userEmail }] 
  });
}
...
export async function getUserBillingHistory(userId) {
  ...
  return db.collection("billing_history").find({
    $or: [
      { userId: clean },
      { userId: cleanLower },
...
export async function validateCoupon(code, planPrice = 0) {
  ...
  if (finalPrice <= 0) {
    finalPrice = 1; // Minimum 1 INR default for 100% discount
  }
...
export async function getAccounts(userId) {
  ...
  // Deduplicate by platform + providerAccountId
  const uniqueMap = new Map();
  accounts.forEach(acc => {
    const key = `${acc.platform || ''}:${acc.providerAccountId || acc._id || (acc.name || '').toLowerCase().trim()}`;
...
export async function getAdAccounts(userId) {
  ...
  const filter = {};
  if (userId) {
    filter.$or = [{ userId }, { user: userId }];
  }
  return db.collection("ad_accounts").find(filter).toArray();
}

export async function upsertAdAccount(adAccount) {
  const db = await getDb();
  const filter = { accountId: adAccount.accountId };
  if (adAccount.userId) {
    filter.userId = adAccount.userId;
  }
  await db.collection("ad_accounts").updateOne(
    filter,
    { $set: { ...adAccount, updatedAt: new Date().toISOString() } },
    { upsert: true }
  );
  return adAccount;
}

export async function removeAdAccount(accountId, userId) {
  const db = await getDb();
  const filter = { accountId };
  if (userId) {
    filter.userId = userId;
  }
  await db.collection("ad_accounts").deleteOne(filter);
}

// ── META AD CAMPAIGNS ──

export async function getAdCampaigns(accountId, userId) {
  const db = await getDb();
  const filter = {};
  if (accountId) {
    filter.$or = [{ accountId }, { adAccountId: accountId }];
  }
  if (userId) {
    filter.userId = userId;
  }
  return db.collection("ad_campaigns").find(filter).sort({ createdAt: -1 }).toArray();
}

export async function upsertAdCampaign(campaign) {
  const db = await getDb();
  const filter = { id: campaign.id };
...
export async function removeAdCampaign(id, userId) {
  const db = await getDb();
  const filter = { id };
  if (userId) {
    filter.userId = userId;
  }
  await db.collection("ad_campaigns").deleteOne(filter);
}
