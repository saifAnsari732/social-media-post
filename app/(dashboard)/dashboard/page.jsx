...
export default function DashboardPage() {
  ...
  const handleAdminToggleBlock = async (targetUserId, currentStatus) => {
  ...
  const handleAdminToggleStatus = handleAdminToggleBlock;
                        ...
                        <tr key={u.id || u.userId} className="hover:bg-slate-50/60 transition-colors">
                          ...
                          <td className="py-3.5 px-4">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10.5px] font-bold border ${
                              ...
                              {!isBlocked ? <CheckCircle2 className="w-3 h-3 text-emerald-600" /> : <Ban className="w-3 h-3 text-rose-600" />}
                          ...
                          <td className="py-3.5 px-5 text-right whitespace-nowrap">
                            <div className="relative inline-flex items-center gap-2 justify-end">
                              ...
                              <button
                                ...
                                className="px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs shadow-2xs transition-all cursor-pointer flex items-center gap-1.5"
                              ...
                              {/* Click-to-open Controls Dropdown */}
                              <div className="relative">
                                <button
                                  ...
                                  title="Click to open tenant actions"
                                >
                                  <span>Manage</span>
                                  ...
                                  <>
                                    ...
                                    <div className="absolute right-0 top-full mt-1.5 w-52 bg-white rounded-2xl border border-slate-200 shadow-xl py-1.5 z-40 text-left">
                                      ...
                                      <button
                                        ...
                                        className="w-full px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-purple-50 hover:text-purple-700 flex items-center gap-2.5 transition-colors cursor-pointer"
                                      >
                                        <ArrowUpRight className="w-4 h-4 text-purple-600" />
                                        <span>Login As (Workspace)</span>
                                      </button>
                                      ...
                                      <button
                                        onClick={() => {
                                          setActionMenuOpenId(null);
                                          handleAdminToggleBlock(tenantId, u.status);
                                        }}
                                      ...
                                      >
                                        ...
                                        <span>{isBlocked ? "Unblock Tenant" : "Block Tenant"}</span>
                                      </button>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 max-w-lg w-full p-6 shadow-xl space-y-5">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2.5">
                  <button
                    ...
                    className="px-3 py-2.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 font-bold hover:bg-emerald-100 transition-all cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <Clock className="w-4 h-4 text-emerald-600" />
                    <span>+7 Days Extension</span>
                  </button>
                  <button
                    onClick={() => handleAdminToggleBlock(inspectingUser.userId || inspectingUser.id, inspectingUser.status)}
                    className={`px-3 py-2.5 rounded-xl border font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                      inspectingUser.status === "Blocked" || inspectingUser.status === "Suspended"
                        ? "bg-emerald-50 border-emerald-200 text-emerald-800 hover:bg-emerald-100"
                        : "bg-rose-50 border-rose-200 text-rose-700 hover:bg-rose-100"
                    }`}
                  >
                    <Ban className="w-4 h-4" />
                    <span>{inspectingUser.status === "Blocked" || inspectingUser.status === "Suspended" ? "Unblock Tenant" : "Block Tenant"}</span>
                  </button>
                </div>

                <button
                  onClick={() => handleImpersonate(inspectingUser)}
                  className="w-full px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-bold cursor-pointer shadow-xs shadow-indigo-600/20 flex items-center justify-center gap-2 transition-all"
                >
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-xs text-slate-500 font-medium">
                  Status: <strong className={inspectingUser.status === "Blocked" || inspectingUser.status === "Suspended" ? "text-rose-600" : "text-emerald-600"}>{inspectingUser.status || "Active"}</strong>
                </span>
