  ...
  return (
    <div className="space-y-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        {/* Status Tabs */}
        <div className="flex items-center gap-1.5 bg-slate-100/80 border border-slate-200 p-1.5 rounded-xl shadow-xs text-xs font-bold text-slate-700">
          {["All", "Published", "Scheduled", "Draft", "Failed"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-3.5 py-2 rounded-lg transition-all ${
                activeTab === tab ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/20 font-extrabold" : "hover:bg-slate-200/60 text-slate-700 font-bold"
              }`}
            >
              {tab}
            </button>
        ...
        {/* Search */}
        <div className="flex items-center gap-2 bg-white border border-slate-300 px-3.5 py-2.5 rounded-xl text-xs max-w-xs w-full shadow-xs focus-within:border-indigo-600 focus-within:ring-2 focus-within:ring-indigo-600/10 transition-all">
          ...
          <input
            ...
            className="w-full border-none bg-transparent outline-none text-slate-900 placeholder:text-slate-400 font-semibold"
          />
        </div>
      </div>
