              ...
              <table className="w-full text-left border-collapse text-xs">
                ...
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  {filteredUsers.map((u) => {
                    ...
                    const isSuperAdmin = u.role === "admin" || (u.email && (u.email.includes("saif") || u.email.includes("ansari")));
                    ...
                    return (
                      <tr key={tenantId} className="hover:bg-slate-50/70 transition-colors">
                        <td className="py-4 px-5">
                          <div className="flex items-center gap-3">
                            <div className="relative shrink-0">
                              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-indigo-600 to-violet-600 text-white flex items-center justify-center font-bold text-xs shadow-xs">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                <h4 className="font-bold text-slate-950 truncate text-xs max-w-[180px]">{u.name}</h4>
                                {isSuperAdmin && (
                                  <span className="px-1.5 py-0.2 rounded bg-indigo-50 text-indigo-700 text-[9px] font-black border border-indigo-200">ADMIN</span>
                                )}
                              </div>
                              <span className="text-[11px] text-slate-500 block truncate max-w-[180px]">{u.email}</span>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-4 whitespace-nowrap">
                          <button
                            onClick={() => handleToggleRole(tenantId, u.role)}
                          ...
                          >
                            {isSuperAdmin ? <ShieldCheck className="w-3.5 h-3.5 text-indigo-600" /> : <User className="w-3.5 h-3.5 text-slate-500" />}
                        ...
                        <td className="py-4 px-4 whitespace-nowrap">
                          <div className="relative inline-block w-48">
                            <select
                              ...
                              onChange={(e) => handleUpdatePlan(tenantId, e.target.value)}
                        ...
                        <td className="py-4 px-5 text-right whitespace-nowrap">
                          <div className="inline-flex items-center gap-2 justify-end">
                            ...
                            <div className="relative">
                              ...
                              {isMenuOpen && (
                                <div className="absolute right-0 top-full mt-1.5 w-48 bg-white rounded-2xl border border-slate-200 shadow-xl py-1.5 z-40 text-left">
                                  <button
                                    onClick={() => {
                                      handleResetTrial(tenantId);
                                      setActionMenuOpenId(null);
                                    }}
                                    className="w-full px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 flex items-center gap-2.5 transition-colors cursor-pointer"
                                  >
                                    <RotateCcw className="w-4 h-4 text-indigo-600" />
                                    <span>Reset 5-Day Timer</span>
                                  </button>

                                  <button
                                    onClick={() => {
                                      handleToggleStatus(tenantId);
                                      setActionMenuOpenId(null);
                                    }}
                                  ...
                                  >
                                    ...
                                    <span>{u.status === "Active" ? "Suspend Tenant" : "Activate Tenant"}</span>
                                  </button>
