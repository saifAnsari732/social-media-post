...
import { Plus, MessageSquareQuote, Play, Pause, MoreVertical } from "lucide-react";
...
export default function CommentsPage() {
  ...
  async function fetchRules() {
    try {
      ...
      // Filter only comment rules
      setRules((data.rules || []).filter(r => r.type === 'comment'));
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="max-w-5xl mx-auto py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-[#0F172A] tracking-tight">Comment Automation</h1>
          <p className="text-[#64748B] text-sm mt-1">Automatically reply to comments on your posts based on keywords.</p>
        </div>
        <Link 
          href="/comments/new"
        ...
        >
          <Plus className="w-4 h-4" /> Create Comment Rule
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-dashed border-[#CBD5E1] p-12 text-center">
          <div className="w-16 h-16 bg-[#F8FAFC] rounded-full flex items-center justify-center mx-auto mb-4 border border-[#E2E8F0]">
            <MessageSquareQuote className="w-8 h-8 text-[#94A3B8]" />
          </div>
          <h3 className="text-lg font-bold text-[#0F172A] mb-2">No comment rules yet</h3>
          <p className="text-[#64748B] max-w-sm mx-auto mb-6">Create your first comment automation rule to start responding to your followers instantly.</p>
          <Link href="/comments/new" className="bg-[#0F172A] text-white px-5 py-2.5 rounded-lg font-medium text-sm inline-flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span className="text-xs font-medium bg-[#F1F5F9] text-[#64748B] px-2 py-0.5 rounded flex items-center gap-1">
                      <MessageSquareQuote className="w-3 h-3" /> Comment
                    </span>
                  </div>
                </div>
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  ...
                  {(!rule.trigger?.keywords || rule.trigger.keywords.length === 0) && (
                    <span className="text-[11px] font-medium bg-[#F1F5F9] text-[#64748B] px-2 py-0.5 rounded-full">Any Comment</span>
                  )}
                </div>
              </div>
